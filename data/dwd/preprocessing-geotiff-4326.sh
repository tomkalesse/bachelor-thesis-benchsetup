...
OUTPUT_DIR="./dwd-geotiff-4326"
OUTPUT_DIR_H1="${OUTPUT_DIR}_H1"
OUTPUT_DIR_H2="${OUTPUT_DIR}_H2"

# Create directories
mkdir -p "$DOWNLOAD_DIR"
mkdir -p "$GEOTIFF_DIR"
mkdir -p "$OUTPUT_DIR"
mkdir -p "$OUTPUT_DIR_H1"
mkdir -p "$OUTPUT_DIR_H2"
...
find "$DOWNLOAD_DIR" -name "*.asc" -print0 | parallel -0 -j 8 '
    ...
    if [[ -n "$datepart" ]]; then
        month=${datepart:4:2}
        if ((10#$month <= 6)); then
            out_tif="'"$OUTPUT_DIR_H1"'/${base_name}.tif"
        else
            out_tif="'"$OUTPUT_DIR_H2"'/${base_name}.tif"
        fi
    else
        echo "(no date found)" >&2
...
tar -czf "${OUTPUT_DIR}_H1.tar.gz" -C "$(dirname "$TMP_H1")" "$(basename "$TMP_H1")"
tar -czf "${OUTPUT_DIR}_H2.tar.gz" -C "$(dirname "$TMP_H2")" "$(basename "$TMP_H2")"

echo "Done. Archives created:"
ls -lh "${OUTPUT_DIR}_H1.tar.gz" "${OUTPUT_DIR}_H2.tar.gz"